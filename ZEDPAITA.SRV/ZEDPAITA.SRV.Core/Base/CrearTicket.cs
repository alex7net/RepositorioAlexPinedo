...
namespace ZEDPAITA.SRV.Core.Base
{
    public class CrearTicket
    {
        ...
        public void EncabezadoVenta()
        {
            ...
            linea.AppendLine("PRODUCTO           |PARTIDA|UND|CANTIDAD");
        }
        ...
        public void TextoExtremosPlaca(string textoIzquierdo, string textoDerecho)
        {
            //variables que utilizaremos
            string textoIzq, textoDer, textoCompleto = "", espacios = "";
        ...
        public void TextoExtremos2(string textoIzquierdo, string textoCentro,string textoDerecho)
        {
            //variables que utilizaremos
            string textoIzq, textoCent, textoDer, textoCompleto = "", espacios = "";
            ...
            if (textoCentro.Length > 18)
            {
                cortar = textoIzquierdo.Length - 18;
                textoCent = textoCentro.Remove(18, cortar);
            }
            else
            { textoCent = textoCentro; }
        ...
        public void AgregaArticulo2(string producto, string partida, string und, int cantidad)
        {
            //Valida que cant precio e importe esten dentro del rango.
            if (partida.ToString().Length <= 10 && und.ToString().Length <= 7 && cantidad.ToString().Length <= 8)
            {
                string elemento = "", espacios = "";
                bool bandera = false;//Indicara si es la primera linea que se escribe cuando bajemos a la segunda si el nombre del articulo no entra en la primera linea
                int nroEspacios = 0;
                ...
                {
                    //Colocar la cantidad a la derecha.
                    nroEspacios = (5 - partida.ToString().Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                        espacios += " ";//Generamos los espacios necesarios para alinear a la derecha
                    }
                    elemento += espacios + partida.ToString();//agregamos la cantidad con los espacios

                    //Colocar el precio a la derecha.
                    nroEspacios = (7 - und.ToString().Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                        espacios += " ";//Genera los espacios
                    ...
                    //el operador += indica que agregar mas cadenas a lo que ya existe.
                    elemento += espacios + und.ToString();//Agregamos el precio a la variable elemento

                    //Colocar el importe a la derecha.
                    nroEspacios = (8 - cantidad.ToString().Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                        espacios += " ";
                    }
                    elemento += espacios + cantidad.ToString();//Agregamos el importe alineado a la derecha
                    ...
                    for (int longitudTexto = producto.Length; longitudTexto > 20; longitudTexto -= 20)
                    {
                        if (bandera == false)//si es false o la primera linea en recorrerer, continuar...
                        {
                            ...
                            linea.AppendLine(producto.Substring(caracterActual, 20));//Solo agrega el nombre del articulo
                ...
                else //Si no es mayor solo agregarlo, sin dar saltos de lineas
                {
                    ...
                    //Colocar la cantidad a la derecha.
                    nroEspacios = (5 - partida.ToString().Length);// +(20 - elemento.Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                        espacios += " ";
                    ...
                    elemento += espacios + und.ToString();
        ...
        public void CortaTicket()
        {
            ...
            linea.AppendLine("\x1B" + "m"); //Caracteres d corte. Estos comando varian segun el tipo de impresora

            // string corte = "\x1B" + "m";                  // caracteres de corte
            // string avance = "\x1B" + "d" + "\x08";
            //  RawPrinterHelper.SendStringToPrinter(impres, "\x1B" + "d" + "\x8"); // avanza
            //  RawPrinterHelper.SendStringToPrinter(impres, "\x1B" + "m"); // corta
